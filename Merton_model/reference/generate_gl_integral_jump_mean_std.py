...
for m in m_list:
    for v in v_list:
        exact_values = exact(Y_nodes, T_grid, mm=m, vv=v)                      # (M, len(t))
        #    build the probability matrix P(y_nodes - x) and multiply it by the Gauss weights
        ...
        prob_matrix = prob_df(diff, mm=m, vv=v) * weights.reshape(1, -1)   # (len(x), M)
        ...
        np.save(fr'gl_integral_{m}_{v}.npy', integral_GL)