...
for N_h in N_hh:
    for N_f in N_ff:
        model = Train(layers_ing_1=layers_ing_1, layers_ing_2=layers_ing_2,
                      layers_pred=layers_pred, params=params, N_h=N_h, N_f=N_f)
        state_dict_1 = torch.load(rf'solution_network_{N_h}_{N_f}.pth', map_location='cpu')
        state_dict_2 = torch.load(rf'integral_network_1_{N_h}_{N_f}.pth', map_location='cpu')
        state_dict_3 = torch.load(rf'integral_network_2_{N_h}_{N_f}.pth', map_location='cpu')
        ...
        u_pred = model.u_pred
        pred_cons = (integral_1(XX, Y_max, TT) - integral_1(XX, XX, TT) +
                     integral_2(XX, XX, TT) - integral_2(XX, Y_min, TT))
...
ax1.set_xlabel(r'$N^1+N^2$', fontsize=20, labelpad=12)
...
ax2.set_xlabel(r'$N^1+N^2$', fontsize=20, labelpad=12)
